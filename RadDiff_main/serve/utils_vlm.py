...
from serve.global_vars import BLIP_FEATURE_URL, BLIP_URL, LLAVA_URL, VLM_CACHE_FILE, CHEXAGENT_URL
...
def get_vlm_output(image: str, prompt: str, model: str) -> str:
    # Normalize path for cache key to ensure consistency across different data locations
    normalized_image = normalize_path_for_hash(image)
    key = json.dumps([model, normalized_image, prompt])
    cached_value = get_from_cache(key, vlm_cache)
    if cached_value is not None:
        logging.debug(f"VLM Cache Hit")
        return cached_value
    ...
    if model in ["blip", "llava", 'chexagent']:
        files = {"image": open(image, "rb").read()}
        text_data = {"text": prompt}
        url = {
            "blip": BLIP_URL,
            "llava": LLAVA_URL,
            "chexagent": CHEXAGENT_URL,
        }[model]
        try:
...
def test_get_vlm_output():
    ...
    question = "Is there a table in the image?"
    answer = vqa(image, question, model)
    question = "Is there a heart in the image?"
    answer = vqa(image, question, model)
    print(f"{answer=}")
