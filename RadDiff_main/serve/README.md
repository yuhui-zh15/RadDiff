...
## CLIP (e.g. ChexZero) Server Configuration

1. Pip install environments: `pip install open-clip-torch flask`
2. Configure global variables in `global_vars.py`
3. Run e.g. `python serve/clip_server.py` `python serve/chexzero_server.py`
4. Run `python -m serve.utils_clip` to test the CLIP.

## MLLM (e.g. ChexAgent) Server Configuration

1. Install environments:
  - BLIP: `pip install salesforce-lavis`
  - LLaVA: `git clone git@github.com:haotian-liu/LLaVA.git; cd LLaVA; pip install -e .`
2. Configure global variables in `global_vars.py`
3. Run e.g. `python serve/vlm_server_[vlm].py` `python serve/vlm_server_chexagent.py`. It takes a while to load the VLM, especially the first time to download the VLM. (Note: concurrency is disabled as it surprisingly leads to worse GPU utilization)
